document.addEventListener("DOMContentLoaded", () => {
    ...
    if (titleEl) {
        if (evaluationType === "ZELF_TUSSENTIJDS") {
            ...
            subtitleEl.textContent = "Je eigen tussentijdse beoordelingen en toelichtingen.";
        } else if (evaluationType === "ZELF_EIND") {
            titleEl.textContent = "Details Zelfreflectie - Eindevaluatie";
            subtitleEl.textContent = "Je eigen eindbeoordelingen en toelichtingen.";
        } else if (evaluationType === "TUSSENTIJDS") {
            titleEl.textContent = "Details Tussentijdse evaluatie";
            subtitleEl.textContent = "Beoordeling en feedback door je stagementor en docent.";
        } else if (evaluationType === "EIND") {
            titleEl.textContent = "Details Eindevaluatie";
            subtitleEl.textContent = "Definitieve beoordeling en feedback door je stagementor en docent.";
        }
    }
    ...
    function populateCategory(gridId, evalData, isSelf, scoreField, feedbackField, scoreDocentField, feedbackDocentField) {
        ...
        if (isSelf) {
            ...
            gridEl.innerHTML = `
                <div class="score-box">
                    <div class="score-box-header">
                        ...
                        <div class="score-value-badge">${evalData[scoreField] || "-"}</div>
                    </div>
                    <p>${evalData[feedbackField] || "Geen toelichting ingevuld."}</p>
                </div>
            `;
        } else {
            ...
            const docentFeedback = evalData[feedbackDocentField];
                ...
                ? `
                    <div class="score-box mentor">
                        <div class="score-box-header">
                            ...
                            <div class="score-value-badge">${mentorScore}</div>
                        </div>
            ...
            const docentBoxHtml = docentScore
                ? `
                    <div class="score-box docent">
                        <div class="score-box-header">
                            <h4>DOCENT (SCHOOL)</h4>
                            <div class="score-value-badge">${docentScore}</div>
                        </div>
                        <p>${docentFeedback || "Geen toelichting ingevuld door docent."}</p>
                    </div>
                `
                : `
                    <div class="score-box docent">
